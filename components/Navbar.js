    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? "bg-[#FAF6F0]/95 backdrop-blur-md shadow-[0_4px_20px_rgba(44,36,32,0.06)] border-b border-[#A3856C]/15 py-1.5"
            : "bg-transparent py-4"
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          {scrolled ? (
            /* Compact Single Row when scrolled */
            <div className="flex items-center justify-between">
              ...
              {/* Center: Nav links */}
              <nav className="hidden md:flex items-center gap-7 text-[10px] uppercase tracking-[0.25em] font-semibold text-[#2C2420]/80">
                {navLinks.map((link, i) => {
                  ...
                  return (
                    <React.Fragment key={link.href}>
                      ...
                      >
                        {link.label}
                        {isActive && (
                          <span className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] w-3 rounded-full bg-[#A3856C]" />
                        )}
                      </a>
                    </React.Fragment>
                  );
              ...
              <a
                ...
                className="inline-flex items-center gap-2 bg-[#2C2420] hover:bg-[#3D2B1F] text-white font-semibold text-[10px] uppercase tracking-[0.18em] px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full shadow-sm transition-all duration-300 hover:shadow-md"
              >
